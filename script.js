...
(function() {
  ...
  var donaldTrumpAttack = ["Legalize drugs to take profit away from drug cartels", "Defend the Second Amendment of our Constitution", "Make America energy independent", "Incentivize employers to provide childcare at the workplace", "Better support for our women veterans", "Build a wall along the Mexican border", "Place a 45% tariff on Chinese exports to the US", "Impose a 20% tax on all imported goods", "Order an immediate review of all US cyber defenses and vulnerabilities", "Close parts of the Internet to prevent ISIS from attracting recruits", "Hillary needs Wall Street money to successfully run her campaign", "She'll say anything and change NOTHING"];
  var hillaryClintonAttack = ["Donald Trump's followers are deplorable", "Just like he shouldn't have his finger on the (nuclear) button, Donald shouldn't have his hands on our economy", "End the privatization of prisons", "Ban military-style weapons", "Provide tuition-free community college", "Increase public investment in clean energy research and development", "Reduce the amount of oil consumed in the US and around the world", "Guarantee up to 12 weeks of paid family and medical leave", "Invest in partnerships in Latin Ameria, Africa, and Asia", "Reduce the cost of prescription drugs", "Improve access to housing and job opportunities", "Work to close the pay gap"];
  //objects
  Hillary = {
    name: "Hillary Clinton",
    attacks: hillaryClintonAttack,
    credibility: 100
  };
  Donald = {
    name: "Donald Trump",
    attacks: donaldTrumpAttack,
    credibility: 100
  };
  //game variables
  ...
  function updateOpponent(opponentCred) {
    console.log(opponentCred);
    $('#opponent .credibility').text(opponentCred);
    currentOpponent.credibility = opponentCred
  }

  function updatePlayer(playerCred) {
    console.log(playerCred);
    $('#player .credibility').text(playerCred);
  ...
  function gameOver() {
    ...
    if (winner === currentPlayer.name) {
      ...
      console.log('gameOver, currentPlayer clause, before jumpforever thing');
  ...
  function playerAttack() { //player has 2 out of 12 chances of missing
    ...
    if (playerAttacks <= 3) {
      ...
      console.log(Number(oldCred));
      var newCred = Number(oldCred) - powerofAttack;
      if (newCred <= 0) {
        getWinner();
      } else {
  ...
  function playerRecover() {
    ...
    console.log(parseInt(oldCred));
    var newCred = Number(oldCred) + recover;
    ...
    console.log('playerRecover, about to removeClass of opponent');
  ...
  function opponentRecover() {
    ...
    console.log(Number(oldCred));
    var newCred = Number(oldCred) + recover2;
  ...
  function opponentAttacks() { //opponent has 2 out of 12 chances of missing
    ...
    if (opponentAttacks <= 3) {
      ...
      console.log('opponentAttacks, about to removeClass of opponent');
      $('#opponent_img').removeClass();
    }
    isOpponentTurn = false;
    $('#attack_btn').show();
